...
pub fn mint_genesis_profile(
...
) -> Result<()> {
    {
        ...
        let collection_state = &mut ctx.accounts.profile_collection_state;
        //verification
        require!(
            !collection_state.is_genesis_profile_init,
            MyError::GenesisNftAlreadyMinted
        );
        //state changes
        //may me all lineage information might be same
        profile_state.mint = ctx.accounts.profile.key();
        profile_state.lineage.creator = lineage.creator;
        profile_state.lineage.parent = lineage.parent;
        profile_state.lineage.grand_parent = lineage.grand_parent;
        profile_state.lineage.greate_grand_parent = lineage.greate_grand_parent;
        profile_state.lineage.uncle_psy = lineage.uncle_psy;
        profile_state.lineage.generation = 1;

        main_state.total_minted_profile += 1;
        collection_state.is_genesis_profile_init = true;
...
pub struct AMintGenesisPeep<'info> {
    ...
    ///CHECK:
    #[account(mut)]
    pub profile_collection: AccountInfo<'info>,

    #[account(
        mut,
        seeds = [SEED_COLLECTION_STATE, profile_collection.key().as_ref()],
        bump,
    )]
    pub profile_collection_state: Box<Account<'info, CollectionState>>,
    ...
    #[account(
        ...
        seeds=[
            METADATA.as_ref(),
            MPL_ID.as_ref(),
            profile_collection.key().as_ref(),
        ],
        bump,
        seeds::program = MPL_ID
    )]
    pub profile_collection_metadata: AccountInfo<'info>,
    ...
    pub profile_collection_edition: AccountInfo<'info>,
...
impl<'info> AMintGenesisPeep<'info> {
    pub fn mint(&mut self, name: String, symbol: String, uri: String) -> Result<()> {
        ...
        let asset_data = AssetData {
            ...
            collection: Some(mpl_token_metadata::state::Collection {
                verified: false,
                key: self.profile_collection.key(),
            }),
            uses: None,
    ...
    pub fn verify_collection_item(&mut self, program_id: &Pubkey) -> Result<()> {
        ...
        let metadata = self.profile_metadata.to_account_info();
        let main_state = &mut self.main_state;
        let collection = self.profile_collection.to_account_info();
        let collection_metadata = self.profile_collection_metadata.to_account_info();
        let collection_edition = self.profile_collection_edition.to_account_info();
