...
pub fn mint_profile(
    ...
    // let mut nft_creators = Vec::<Creator>::new();
    {
        ...
        let minting_cost = main_state.profile_minting_usdc_price;
        ...
        if receiver_ata.owner != user_key {
            transfer_tokens(
                ...
                (minting_cost as u128 * main_state.royalty_for_minting.creator as u128 / 100u128) as u64
            )?;
        }

        transfer_tokens(
            from.clone(),
            ctx.accounts.parent_vault_usdc_ata.to_account_info(),
            user.clone(),
            token_program.to_account_info(),
    (minting_cost as u128 * main_state.royalty_for_minting.parent as u128 / 100u128) as u64
        )?;

        transfer_tokens(
            from.clone(),
            ctx.accounts.grand_parent_vault_usdc_ata.to_account_info(),
            user.clone(),
            token_program.to_account_info(),
    (minting_cost as u128 * main_state.royalty_for_minting.grand_parent as u128 / 100u128) as u64
        )?;
